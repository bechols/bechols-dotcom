...
async function getAPIShelfCount(shelf) {
  ...
  try {
    log(`🌐 API Request: ${options.url}?${new URLSearchParams(options.params)}`)
...
async function syncShelfData(shelf) {
  ...
  while (hasMorePages) {
    ...
    try {
      ...
      const response = await axios(options)
      log(`📡 Response status: ${response.status}, length: ${response.data.length} characters`)
      await delay(1000) // Rate limiting
  ...
  for (const element of allBooks) {
    ...
    try {
      ...
      if (!bookData.goodreads_id || !bookData.title || !bookData.author) {
        log(`⏭️  Skipping book - missing required data: id=${bookData.goodreads_id}, title="${bookData.title}", author="${bookData.author}"`)
        continue
      }
      
      log(`📖 Processing: "${bookData.title}" by ${bookData.author} (${bookData.goodreads_id})`)
      ...
      log(`📝 Book insert result: lastInsertRowid=${bookResult.lastInsertRowid}, changes=${bookResult.changes}`)
      ...
      if (bookData.goodreads_id) {
        ...
        log(`📊 Review insert result: lastInsertRowid=${reviewResult.lastInsertRowid}, changes=${reviewResult.changes}`)
        syncedCount++
      }
      ...
      if (verbose) {
        console.error(`🔍 Book data that caused error:`, JSON.stringify(bookData, null, 2))
  ...
  if (allBooks.length > 0) {
    ...
    log(`🧹 API returned ${apiGoodreadsIds.length} books: ${apiGoodreadsIds.join(', ')}`)
    ...
    if (orphanedBooks.length > 0) {
      console.log(`🧹 Found ${orphanedBooks.length} orphaned books to remove from ${shelf} shelf:`)
      ...
      for (const orphan of orphanedBooks) {
        ...
        console.log(`   🗑️  Removed "${orphan.title}" by ${orphan.author} (${deleteResult.changes} records)`)
      }
    } else {
